...
$logPath = Join-Path $env:LOCALAPPDATA "LOUPe\beta-0.1.4-install.log"
...
function Get-ModelsDirectory {
  ...
  if ([string]::IsNullOrWhiteSpace($directory)) {
    $directory = Join-Path $env:LOCALAPPDATA "LOUPe\beta-0.1.4\models"
  }
...
Log "LOUPe beta 0.1.4 : préparation des composants sélectionnés."
