...
$logPath = Join-Path $env:LOCALAPPDATA "LOUPe\beta-0.1.5-install.log"
...
function Get-ModelsDirectory {
  ...
  if ([string]::IsNullOrWhiteSpace($directory)) {
    $directory = Join-Path $env:LOCALAPPDATA "LOUPe\beta-0.1.5\models"
  }
...
Log "LOUPe beta 0.1.5 : préparation des composants sélectionnés."
